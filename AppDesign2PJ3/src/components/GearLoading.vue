<template>
    <div class="loading-overlay">
        <div class="loading-gear-container">
            <svg class="gear-icon" xmlns="http://www.w3.org/2000/svg" height="60px" viewBox="0 -960 960 960" width="60px" fill="#fff">
            <path d="M452-149q-34.38 0-58.85-24.33-24.48-24.34-24.48-58.7t24.48-58.83q24.47-24.47 58.85-24.47l24.5-23.5q8.17-8.17 19.1-13 10.94-4.84 22.4-4.84 24.33 0 41.17 17.34Q576-322 576-297.85V-280q0 20 14 34t34 14q15.54 0 27.96-9.37 12.43-9.37 17.61-23.94L682-299.67q9-25.33 29.69-41.18 20.69-15.85 47.6-15.85 11.38-28.63 17.88-59.97 6.5-31.33 6.5-64 0-92.33-45.84-167.83Q692-724 617.33-768v38.67q0 34.1-24.28 58.38t-58.38 24.28h-41.34v83.34q0 17-12.16 29.16Q469-522 452-522h-41.33v70.01q0 22.66-15.58 38.32Q379.51-398 356.67-398q-14 0-26.17-6.33-12.17-6.34-19.83-17.34L244.61-522h-41.28v41.13Q203.33-449 182-426q-21.33 23-51.67 27Q157.67-291.67 247-220.33 336.33-149 452-149Zm82.97-249q-17.3 0-29.47-12.17-12.17-12.16-12.17-29.16t12.17-29.17q12.17-12.17 29.47-12.17h40.72q17.31 0 29.48 12.17 12.16 12.17 12.16 29.17t-12.16 29.16Q593-398 575.69-398h-40.72Zm96.29-124q-20.26 0-32.76-16.17-12.5-16.16-6.12-35.77l15.95-45.39q4.67-12.67 14.8-20 10.13-7.34 22.28-7.34 20.26 0 32.76 16.17 12.5 16.17 6.12 35.77l-15.96 45.4q-4.66 12.66-14.79 20-10.13 7.33-22.28 7.33ZM452.18-80q-82.83 0-155.67-31.5-72.84-31.5-127.18-85.83Q115-251.67 83.5-324.56T52-480.33q0-82.88 31.5-155.78Q115-709 169.33-763q54.34-54 127.23-85.5T452.33-880q82.88 0 155.78 31.5Q681-817 735-763t85.5 127Q852-563 852-480.18q0 82.83-31.5 155.67Q789-251.67 735-197.46q-54 54.21-127 85.84Q535-80 452.18-80Z"/>
            </svg>
            <div class="airplane-wrapper">
                <svg class="airplane-icon" xmlns="http://www.w3.org/2000/svg" height="30px" viewBox="0 -960 960 960" width="30px" fill="#FFD964">
                    <path d="M283.33-80v-88.67L406-254v-162.67L80-285.33v-108L406-622v-184q0-30.33 21.83-52.17Q449.67-880 480-880q30.33 0 52.17 21.83Q554-836.33 554-806v184l326 228.67v108L554-416.67V-254l122 85.33V-80l-196-59.33L283.33-80Z"/>
                </svg>
            </div>
        </div>
        <p class="loading-text">Loading...</p>
    </div>
</template>

<style scoped>
.loading-overlay {
  position: fixed;
  top: 0;
  left: 0;
  width: 100vw;
  height: 100vh;
  background-color: rgba(0, 0, 0, 0.6);
  display: flex;
  flex-direction: column;
  justify-content: center;
  align-items: center;
  z-index: 9999;
}

.loading-gear-container {
  position: relative;
  width: 60px;
  height: 60px;
}

.gear-icon {
  width: 100%;
  height: 100%;
  animation: rotateGear 4s linear infinite;
}

.airplane-wrapper {
  position: absolute;
  top: -10px;
  left: 50%;
  width: 30px; /* アイコンのサイズと合わせる */
  height: 30px;
  animation: rotateAroundGear 4s linear infinite;
  transform-origin: 50% calc(50% + 40px);
}

.airplane-icon {
  transform: rotate(90deg);
}

.loading-text {
  color: #fff;
  margin-top: 1rem;
  font-size: 1.1rem;
  font-weight: 500;
  letter-spacing: 1px;
}

@keyframes rotateGear {
  from {
    transform: rotate(0deg);
  }
  to {
    transform: rotate(360deg);
  }
}

@keyframes rotateAroundGear {
  from {
    transform: translateX(-50%) rotate(0deg);
  }
  to {
    transform: translateX(-50%) rotate(360deg);
  }
}
</style>